...
const DonationsTable: React.FC<DonationsTableProps> = ({ filter, data }) => {
  ...
  const filterData = () => {
    return data.filter((donation) => {
      return (
        (!filter.causes ||
          filter.causes.length === 0 ||
          filter.causes.includes(donation.Cause)) &&
        (!filter.frequencies ||
          filter.frequencies.length === 0 ||
          filter.frequencies.includes(donation.Frequency)) &&
        (!filter.years ||
          filter.years.length === 0 ||
          filter.years.includes(
            new Date(donation.Date).getFullYear().toString(),
          ))
      );
    });
  };
  ...
  const changePage = (offset: number) => {
    setCurrentPage((prev) =>
      Math.max(0, Math.min(pageCount - 1, prev + offset)),
    );
  };
    ...
    <TableContainer padding="16px">
      ...
      >
        <Table variant="striped" colorScheme="gray" color="#4E5153">
          <Thead>
            <Tr bgColor="#fadbe7">
              <Th textAlign="left" textTransform="none" padding="16px">
                Cause
              </Th>
      ...
      <Box position="relative" zIndex="1">
        ...
        >
          ...
          <Text>
            {startEntry}-{endEntry} of {filteredData.length}
          </Text>
          <Button
